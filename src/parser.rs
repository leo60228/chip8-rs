...
pub fn instr_bits(input: Bits) -> IResult<Bits, Instruction> {
    ...
    alt((
        ...
        map(preceded(bits::tag(0x0, 4usize), addr_bits), |addr| RcaCall(addr)),
        map(preceded(bits::tag(0x1, 4usize), addr_bits), |addr| Goto(addr)),
        map(preceded(bits::tag(0x2, 4usize), addr_bits), |addr| Call(addr)),
        map(preceded(bits::tag(0x3, 4usize), tuple((register_bits, bits::take(8usize)))),
            |(reg, n): (Register, u8)| SkipEqImm(reg, n)),
        map(preceded(bits::tag(0x4, 4usize), tuple((register_bits, bits::take(8usize)))),
            |(reg, n): (Register, u8)| SkipNeqImm(reg, n)),
        map(preceded(bits::tag(0x5, 4usize), tuple((register_bits, register_bits, bits::take(4usize)))),
            |(r1, r2, _): (_, _, u8)| SkipEqReg(r1, r2)),
        map(preceded(bits::tag(0x6, 4usize), tuple((register_bits, bits::take(8usize)))),
            |(reg, n): (Register, u8)| SetImm(reg, n)),
        map(preceded(bits::tag(0x7, 4usize), tuple((register_bits, bits::take(8usize)))),
            |(reg, n): (Register, u8)| AddImm(reg, n)),
        map(preceded(bits::tag(0x8, 4usize), tuple((register_bits, register_bits,
                     bits::tag(0x0, 4usize)))),
        ...
        map(preceded(bits::tag(0x9, 4usize), tuple((register_bits, register_bits,
                     bits::tag(0x0, 4usize)))),
            |(r1, r2, _): (_, _, u8)| SkipNeqReg(r1, r2)),
    alt((
        map(preceded(bits::tag(0xA, 4usize), addr_bits), |addr| SetAddr(addr)),
        map(preceded(bits::tag(0xB, 4usize), addr_bits), |addr| IndexedJump(addr)),
        map(preceded(bits::tag(0xC, 4usize), tuple((register_bits, bits::take(8usize)))),
            |(reg, n): (Register, u8)| Rand(reg, n)),
        map(preceded(bits::tag(0xD, 4usize), tuple((register_bits, register_bits,
                     bits::take(4usize)))),
            |(r1, r2, h): (_, _, u8)| Draw(r1, r2, h)),
        map(preceded(bits::tag(0xE, 4usize), 
                     terminated(register_bits, bits::tag(0x9E, 8usize))),
            |reg| SkipPressed(reg)),
        map(preceded(bits::tag(0xE, 4usize), 
                     terminated(register_bits, bits::tag(0xA1, 8usize))),
            |reg| SkipUnpressed(reg)),
        map(preceded(bits::tag(0xF, 4usize), 
